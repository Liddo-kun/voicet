...
impl IncrementalMel {
    ...
    pub fn with_left_context(filters: &[f32], left_context: &[f32]) -> Self {
        let hann: Vec<f32> = (0..N_FFT)
            .map(|i| 0.5 * (1.0 - (2.0 * std::f32::consts::PI * i as f32 / N_FFT as f32).cos()))
            .collect();
        let mut sample_buf = Vec::with_capacity(left_context.len() + 16000);
        sample_buf.extend_from_slice(left_context);
        Self {
            sample_buf,
            hop_pos: 0,
            hann,
            filters: filters.to_vec(),
            initialized: true, // skip reflect padding
        }
    }
    ...
    pub fn drain_frames(&mut self) -> Vec<[f32; N_MELS]> {
        ...
        // Periodically trim consumed samples to prevent unbounded growth.
        // Keep everything from hop_pos onward (needed for the next FFT window).
        if self.hop_pos > 16000 {
            self.sample_buf.drain(..self.hop_pos);
            self.hop_pos = 0;
        }
        frames
    }
